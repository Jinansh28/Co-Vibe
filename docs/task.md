...
Current Task:       TASK-001 — Monorepo Workspace & TypeScript Base Setup
...
### Acceptance Criteria
- [ ] `pnpm install` resolves dependencies across all workspaces without warnings.
- [ ] `pnpm typecheck` executes `tsc --noEmit` across packages with 0 errors.
- [ ] Root `packages/shared` exports core TypeScript interfaces (`User`, `Project`, `Workspace`).

### Tests
- [ ] `packages/shared/tests/types.test.ts` verifies type exports and object initialization.
...
### Acceptance Criteria
- [ ] `OllamaProvider.stream()` yields text chunks as generated by local Ollama model.
- [ ] Model request formats system prompt, message history, and tool definitions correctly.

### Tests
- [ ] `packages/agent/tests/ollama.test.ts` tests mock and live model responses.
...
### Acceptance Criteria
- [ ] Querying context engine for symbol `AuthMiddleware` retrieves `apps/api/src/middleware/auth.ts`.
- [ ] Retrieved context payload fits within configured 40% token budget limit.

### Tests
- [ ] `packages/context/tests/retrieval.test.ts` verifies symbol ranking and budget capping.
